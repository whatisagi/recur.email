    ...
    <DashboardShell>
      ...
      <div className="grid gap-6">
        <DashboardNav
          ...
          currentPath={"/dashboard/" + newsletter.username}
        />
        <div className="px-2">
          <p>
            You can write to{" "}
            <a
              ...
              href={`mailto:${newsletter?.inboundAddress}`}
            >
              {newsletter?.inboundAddress}
            </a>{" "}
            to send a new issue to {subscribersCount} active subscribers.
            <br />
            Only write from{" "}
            ...
            .
            <br />
            People can subsribe at{" "}
            <a
              className="text-blue-500"
              href={`https://${newsletter?.username}.recur.email`}
            >{`${newsletter?.username}.recur.email`}</a>
            .
          </p>
        </div>
        <div className="grid gap-4 px-2 md:grid-cols-2 lg:grid-cols-3">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                +
                {numeral(subscribersCountInLast7Days / subscribersCount).format(
                  "0.00%"
                )}{" "}
                from last month
              </p>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                +
                {numeral(
                  pendingSubscribersCountInLast7Days / pendingSubscribersCount
                ).format("0.00%")}{" "}
                from last month
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {unsubscribedSubscribersCount}
              </div>
              <p className="text-xs text-muted-foreground">
                +
                {numeral(
                  unsubscribedSubscribersCountInLast7Days /
                    unsubscribedSubscribersCount
                ).format("0.00%")}{" "}
                from last month
              </p>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                +{numeral(emailsCountInLast7Days / emailCount).format("0.00%")}{" "}
                from last month
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                +
                {numeral(
                  outboundEmailsCountInLast7Days / outboundEmailsCount
                ).format("0.00%")}{" "}
                from last month
              </p>
            </CardContent>
